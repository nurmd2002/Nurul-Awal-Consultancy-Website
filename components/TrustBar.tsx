const trustItems = [
  {
    icon: (
      <svg viewBox="0 0 64 64" className="h-14 w-14 max-md:h-11 max-md:w-11" fill="none" stroke="currentColor" strokeWidth="2">
    ...
    <section className="relative overflow-hidden bg-[#fbf7ef] px-8 py-20 max-md:px-5 max-md:py-14">
      ...
      <div className="relative z-10 mx-auto max-w-[1320px]">
        <div className="text-center">
          <div className="mb-5 flex items-center justify-center gap-5 max-md:gap-3">
            <span className="h-px w-28 bg-[#c6aa47] max-md:w-10" />
            <span className="h-3 w-3 rotate-45 bg-[#c6aa47] max-md:h-2 max-md:w-2" />
            <p className="font-serif text-lg font-bold uppercase tracking-[0.14em] text-[#c6aa47] max-md:text-xs max-md:leading-6">
          ...
          <div className="mx-auto mt-6 flex w-32 items-center justify-center gap-3 max-md:w-24">
        ...
        <div className="mt-14 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 max-md:mt-10 max-md:gap-6">
          {trustItems.map((item, index) => (
            <div
              key={item.title}
              className={`px-12 py-6 text-center max-md:rounded-[24px] max-md:border max-md:border-[#d8b75b]/25 max-md:bg-white max-md:px-6 max-md:py-8 max-md:shadow-[0_14px_45px_rgba(0,0,0,0.06)] ${
                index !== trustItems.length - 1
                  ? "lg:border-r lg:border-[#d8c680]/35"
                  : ""
              }`}
            >
              <div className="mx-auto flex h-24 w-24 items-center justify-center rounded-full bg-[#177D3F]/9 text-[#063f24] max-md:h-20 max-md:w-20">
                {item.icon}
              </div>

              <div className="mx-auto mt-7 flex w-24 items-center justify-center gap-3 max-md:mt-5">
                <span className="h-px flex-1 bg-[#c6aa47]/70" />
                <span className="h-3 w-3 rotate-45 bg-[#c6aa47]" />
                <span className="h-px flex-1 bg-[#c6aa47]/70" />
              </div>

              <h3 className="mt-6 font-serif text-[30px] font-bold leading-tight text-[#063f24] max-md:text-2xl">
                {item.title}
              </h3>

              <p className="mx-auto mt-5 max-w-[260px] text-[17px] leading-8 text-neutral-700 max-md:text-base max-md:leading-7">
                {item.description}
              </p>
            </div>
          ))}
        ...
        <div className="mx-auto mt-10 flex max-w-5xl items-center justify-center gap-6 max-md:mt-8">
          <span className="h-px flex-1 bg-[#d8c680]/55" />
          <span className="text-5xl text-[#177D3F] max-md:text-3xl">✥</span>
          <span className="h-px flex-1 bg-[#d8c680]/55" />
        </div>
      </div>
    </section>
